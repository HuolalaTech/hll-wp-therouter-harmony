...
const FILE_DOES_NOT_EXIST = 'The source file does not exist.';
class HtmlReport {
    ...
    writeReport(reportNode, template) {
        ...
        fs.writeFileSync(path.join(myPath, INDEX_HTML), template, 'utf8');
    }
    ...
    simplifiedReportTree(node) {
        ...
        if (node.children.size === 1) {
            ...
            if (child.name.endsWith('.ets') || child.name.endsWith('.ts') || child.name.endsWith('.js')) {
                return;
            }
    ...
    onDetail(fileData, filePath) {
        ...
        let fileContent = fs.existsSync(fileData.path) ? fs.readFileSync(fileData.path, 'utf8') : FILE_DOES_NOT_EXIST;
        ...
        fs.writeFileSync(path.join(this.output, filePath), template, 'utf8');
    }
