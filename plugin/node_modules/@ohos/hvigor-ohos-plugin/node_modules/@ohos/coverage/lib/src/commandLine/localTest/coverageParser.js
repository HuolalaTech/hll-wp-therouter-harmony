    ...
    for await (const [idx, line] of logFileArr.entries()) {
        ...
        if (coverageType === 'ohosTest' && line.indexOf(OHOS_REPORT_COVERAGE_PATH) != -1) {
            ...
            if (isAtomicServiceProject(coverageParserParam.bundleType)) {
                command = `${hdcPath} file recv -b ${coverageParserParam.bundleName} ls ${NEW_SAND_BOX_PATH} ${coverageDirectory}`;
            }
            else {
                command = `${hdcPath} file recv ${pathInDevice} ${coverageDirectory}`;
            }
            (0, child_process_1.execSync)(command, { windowsHide: true });
            continue;
        }
    }
